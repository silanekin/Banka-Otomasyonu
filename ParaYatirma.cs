...
namespace banka_otomasyonu
{
    public partial class ParaYatirma : Form
    {
        ...
        private void btnOnay_Click(object sender, EventArgs e)
        {
            try
            {
                var paraYatir = new ParaYatirmaInputModel()
                {
                    HesapNo = lblHesapBilgisi.Tag != null ? lblHesapBilgisi.Tag.ToString() : "",
                    Tutar = txtTutar.Text
                };

                MusteriBusiness.ParaYatir(paraYatir);

                lblHesapBilgisi.Tag = null;
                lblHesapBilgisi.Text = string.Empty;
                txtTutar.Text = string.Empty;
                MessageBox.Show("Para yatırma işlemi başarıyla gerçekleştirilmiştir.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtTutar.Focus();
            }
            catch (Exception Ex)
            {
        ...
        private void ParaYatirma_FormClosing(object sender, FormClosingEventArgs e)
        {
